...
var MAX_PRECISION_FOR_OPERATIONS = 15;
...
function safeDivideBy(a, b) {
  var precision = Math.min(getPrecision(a) + getPrecision(b), MAX_PRECISION_FOR_OPERATIONS);
...
function getClosestPoint(val, _ref2) {
  ...
  if (step !== null) {
    var maxSteps = Math.floor(safeDivideBy(max - min, step));
    var steps = Math.min(safeDivideBy(val - min, step), maxSteps);
    var closestStep = safeMultiply(Math.round(steps), step) + min;
...
function ensureValuePrecision(val, props) {
  ...
  return step === null ? closestPoint : withPrecision(closestPoint, getPrecision(step));
}
